...
public class SurfaceClosestPoint3dMap<C extends VertexToMeshCorrespondence> 
extends GeneralizedClosestPoint3dMap <C> {
   
   double srcCov = 0.4;

   public SurfaceClosestPoint3dMap () {
      super ();
   }
   ...
   protected SparseBlockMatrix createWeightMatrix (
      VertexToMeshCorrespondence correspondence, boolean forSource, int index) {
      ...
      || (!correspondence.getDirection () && !forSource)) {
         ...
         if (correspondence.getNearestFeature () instanceof Face) {
            Face face = (Face)correspondence.getNearestFeature ();
