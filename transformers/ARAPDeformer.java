...
public class ARAPDeformer extends MeshBasedLinearDeformer{
   ...
   public VectorNd creatTargetForce () {
      ...
      pointMap = this.creatPointMap ();
      for (PolygonalMesh mesh: myMeshes) {
         int vtxIdx = 0;
         double [] areas = myVoronoi.get (mesh);
         for (Vertex3d vtx : mesh.getVertices ()) {
            Matrix3d CellR = new Matrix3d(GeometryOperator.
               computeCellLocalRotation (vtx, pointMap, myLs.get (mesh)));
            CellR.scale (1.0 / areas[vtxIdx]);
            vtxIdx ++;
